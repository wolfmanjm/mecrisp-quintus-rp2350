...
var memory : array[$20000000..$20081FFF] of byte;

function read8 (addr : dword) : dword;
begin

  case addr of
    $40070000: read8 := key; // UART Data Register
  else
    read8 := memory[addr];
  end;
...
begin
  case addr of
    $40070000: emit(data and $FF);  // UART Data Register
  else
...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory[$20000000], data - $20000000);
                 fileclose(coredump);
                 halt;
               end;
  else
...
begin
  if (read16(get_pc) and 3) < 3 then printopcode :=  word2hex(                             read16(get_pc)) + '    '
                                else printopcode := dword2hex((read16(get_pc+2) shl 16) or read16(get_pc) ) ;
end;
...
begin
  ...
  fileread(binaryimage, memory[$20000000], binarysize);
  ...
  // for i := 1 to 2000000 do // Maximum amount of instructions...
  // begin
  repeat
